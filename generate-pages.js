...
<head>
    ...
    <title>i-HIT - ${data['Item Name']} Details</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">Instant Halal & Inventory Tracker (i-HIT)</div>
        <div class="item-name">${data['Item Name']}</div>
        
        ${Object.entries(data)
          .filter(([key]) => !['Today', 'Certificate Expired in (Day)'].includes(key))
          .map(([key, value]) => {
            if (key === 'Invoice' || key === 'Halal Certificate') {
              return `<div class="detail-row">
                <div class="detail-label">${key}:</div>
                <div class="detail-value">
                  ${value === 'NA' ? 'NA' : `<a href="${value}" class="btn btn-blue">View ${key.includes('Certificate') ? 'Certificate' : 'Invoice'}</a>`}
                </div>
              </div>`;
            }
            return `<div class="detail-row">
              <div class="detail-label">${key}:</div>
              <div class="detail-value">
                ${key === 'Halal Certificate' ? `<span class="${value === 'Available' ? 'cert-available' : 'cert-not-available'}">${value}</span>` : 
                 key === 'Certificate Expiry Date' ? `<span class="${value === 'NA' ? '' : (data['Certificate Expired in (Day)'] === '#NUM!' ? 'expired' : 'valid')}">${value}${value === 'NA' ? '' : ` (${data['Certificate Expired in (Day)'] === '#NUM!' ? 'Expired' : 'Valid'})`}</span>` : 
                 value}
              </div>
            </div>`;
          }).join('')}
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>i-HIT - Inventory List</title>
    <style>
        * { box-sizing: border-box; }
        body { margin: 0; padding: 15px; background-color: #f5f5f5; font-family: Arial, sans-serif; }
        .header { font-family: "Century Gothic", CenturyGothic, AppleGothic, sans-serif; color: #0066cc; text-align: center; font-size: 22px; font-weight: bold; margin-bottom: 25px; }
        ...
        <div class="item-card">
            ...
            <div class="item-details">
                ...
                <div class="item-value ${item['Halal Certificate'] === 'Available' ? 'cert-available' : 'cert-not-available'}">
                    ${item['Halal Certificate']}${item['Certificate Expiry Date'] !== 'NA' && item['Certificate Expired in (Day)'] === '#NUM!' ? ' (Expired)' : ''}
                </div>
            </div>
